...
onBeforeMount(() => {
  if(!reviewStore.getMyRelatedReviews.length!=0){
    reviewStore.fetchMyRelatedReviews();
  }
});
const current_page = ref(1);
const handle_page = (page)=>{
  if(page < 1 || page > Math.ceil(reviewStore.getMyRelatedReviews.length/7)) return;
  current_page.value = page;
}
const items = computed(()=>{
  return reviewStore.getMyRelatedReviews.slice((current_page.value-1)*7,current_page.value*7);
});
